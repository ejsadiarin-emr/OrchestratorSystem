...
| Agent CLI | `System.CommandLine`-style args | `--enroll`, `--orchestrator-url`, `--display-name`, `--reset-enrollment` |
...
Written by `DeploymentPoC.Agent.exe --enroll`. Deleted by `DeploymentPoC.Agent.exe --reset-enrollment`. Not manually edited.
...
```
DeploymentPoC.Agent.exe --reset-enrollment
```
...
| Agent reset (--reset-enrollment, deletes config only) | Yes | |
...
7. Agent `--reset-enrollment` CLI mode (delete agent.json)
...
| 5 | Agent identity | GUID (`NodeId`) generated by Orchestrator at enrollment, stored in `agent.json`. Deleted on `--reset-enrollment`. Not hostname-based. |
...
| 23 | `--reset-enrollment` behavior | Only deletes `agent.json`. Does NOT call any API or touch the Windows Service. The Orchestrator detects node absence via heartbeat timeout. |
