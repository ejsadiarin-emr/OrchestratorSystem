...
- Workloads (/workloads):
    ...
    - Backend: Workloads have JSON schema (manifest/metadata) that is pre-defined already (see sample schema: ../sample-workload-definition.jsonc ) - note that for this PoC have like 2-3 packages/artifacts only per workload
