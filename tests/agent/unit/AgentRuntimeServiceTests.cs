...
{
    [Test]
    public async Task ExecuteAsync_LogsError_WhenNodeIdIsMissing()
    {
        ...
        var service = new AgentRuntimeService(config, loggerMock.Object, pipelineFake, httpFactory);
        ...
        loggerMock.Verify(
            x => x.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("NodeId")),
                It.IsAny<Exception?>(),
    ...
    public async Task FireAndForget_TerminalPatch_Sent_Even_When_StoppingToken_Cancelled()
    {
        ...
        var service = new AgentRuntimeService(config, loggerMock.Object, pipelineExecutor, httpFactoryMock.Object);
